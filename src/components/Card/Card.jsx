...
let rating = 0
  ...
  return (
    <div className={`card ${showCard && 'hoverBtn'}`}
      onMouseEnter={() => setShowCard(true)}
      onMouseLeave={() => setShowCard(false)}
    >
      {(showCard && isMobile) && (
        <div className='btnContainer'>
          <button className={`btn ${showCard &&'hoverBtn'}`} onClick={() => {
            dispatch(increment())}
            }>
            Add to Cart
          </button>
    ...
    <StarIcon
      key={num}
      className={`starIcon ${
        num < rating ? 'fillStar' : 'emptyStar'
      }`}
    />
  ))}
  </div>
          {(!isMobile && showCard) && (
            <button className={`mobBtn ${showCard && 'hoverBtn'}`} onClick={() => {
              dispatch(increment())}
              }>
              Add to Cart
            </button>
